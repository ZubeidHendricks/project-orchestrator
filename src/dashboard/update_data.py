...
def update_dashboard_data():
    ...
    for issue in open_issues:
        if not issue.assignee:
            ...
            available_devs = [dev_issue.assignee for dev_issue in dev_issues if dev_issue.assignee]
            
            if available_devs:
                # Assign to least busy developer
                dev_counts = {}
                for dev in available_devs:
                    dev_counts[dev.login] = sum(1 for i in dev_issues if i.assignee and i.assignee.login == dev.login)
