...
def details(repo_name):
    ...
    manager = RepositoryManager()
    try:
        repo = manager.get_repository_details(repo_name)
        click.echo("Repository Details:")
        click.echo(f"Name: {repo['name']}")
        click.echo(f"Owner: {repo['owner']}")
        click.echo(f"Category: {repo.get('category', 'uncategorized')}")
        click.echo(f"Priority: {repo.get('priority', 'medium')}")
        click.echo(f"Status: {repo.get('status', 'active')}")
        if 'github_data' in repo:
            click.echo("\nGitHub Stats:")
            click.echo(f"Open Issues: {repo['github_data']['open_issues']}")
            click.echo(f"Stars: {repo['github_data']['stars']}")
            click.echo(f"Forks: {repo['github_data']['forks']}")
    except Exception as e:
        click.echo(f"Error: {str(e)}", err=True)
