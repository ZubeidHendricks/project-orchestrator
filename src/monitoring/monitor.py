...
class ProjectMonitor:
    ...
    def run_monitoring(self):
        ...
        results = {"timestamp": datetime.now().isoformat(), "projects": {}}

        for project_type, config in self.project_types.items():
            for repo_name in config["repositories"]:
                results["projects"][repo_name] = self.check_repository(
                    repo_name, project_type
                )
    ...
    def get_health_metrics(self, repo, project_type):
        ...
        try:
            ...
            if recent_runs:
                success_rate = (
                    len([r for r in recent_runs if r.conclusion == "success"])
                    / len(recent_runs)
                    * 100
                )
            else:
    ...
    def check_database_health(self, repo):
        ...
        try:
            ...
            db_files = [
                f
                for f in contents
                if any(x in f.path.lower() for x in ["database", "migration", "schema"])
            ]
